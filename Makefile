...
screen:
	bundle exec generamba gen $(modName) surf_mvp_coordinatable_module --module_path 'TargetsCheck/Flows/$(flow)' --test_path 'TargetsCheckTests/Tests/Flows/$(flow)'

## Used to create a new alert. Example: make alert modName=<AlertName> flow=<FlowName>
alert:
	bundle exec generamba gen $(modName) surf_mvp_coordinatable_alert --module_path 'TargetsCheck/Flows/$(flow)' --test_path 'TargetsCheckTests/Tests/Flows/$(flow)'
...
synx:
	bundle exec synx --exclusion "TargetsCheck/Non-iOS Resources" TargetsCheck.xcodeproj
